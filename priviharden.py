...
class HardenReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "PriViHarden Linux Security Audit Report",new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.set_xy(10, 20)
...
def generate_pdf(auditor: PriViHarden, operator: dict = None) -> str:
    score     = max(0, auditor.score)
    ...
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_text_color(60, 60, 60)
    pdf.cell(50, 7, "  Security Score Bar:",new_x=XPos.RIGHT, new_y=YPos.TOP)
    bx = pdf.get_x()
    by = pdf.get_y()
    ...
    for cat, checks in categories.items():
        ...
        for r in checks:
            ...
            pdf.cell(25, 6, f"  {r.status}", fill=True,new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.set_text_color(0, 0, 0)
            ...
            pdf.cell(0, 5, f"    {detail_clean}",new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_text_color(0, 0, 0)
            pdf.ln(1)
        pdf.ln(3)
    ...
    if non_secure:
        ...
        for i, r in enumerate(non_secure, 1):
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(80, 80, 80)
            pdf.cell(0, 6, f"  {i}. [{r.status}] {r.name}",new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "", 8)
        ...
        pdf.cell(0, 6, "  No remediation required  -  system is well hardened.",new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.multi_cell(
        ...
        "\n\nThe audit was performed on the local system under the authorization of the system operator.\n\n"
...
def authorization_gate():
    ...
    console.print(Panel(gate_text, border_style="red",
                        title="[bold red]PriViHarden | PriViSecurity[/bold red]"))
...
def main():
    ...
    try:
        pdf_file = generate_pdf(auditor, operator)
        if pdf_file:
